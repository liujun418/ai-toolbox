...
export default function AvatarGeneratorClient({ locale = "en" as Locale, dict }: { locale?: Locale; dict?: Record<string, unknown> }) {
  ...
  const tool = useTool({
    ...
    buildPrompt: ({ style, customPrompt }) => {
      ...
      return `${style} avatar style. ${desc}`.trim();
    },
    locale,
    dict,
  });
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6">
      <div className="mb-8">
        ...
        <p className="mt-2 text-sm text-zinc-500 dark:text-zinc-400">{t.description || "Transform your photo into cartoon, anime, or professional avatars."} <span className="font-semibold text-blue-600">{t.cost || `${CREDIT_COST} credits`}</span>.</p>
      </div>
            ...
            {tool.status === "idle" && (
              <div className="flex gap-3">
                <button onClick={() => tool.handleUpload({ style: selectedStyle, customPrompt: prompt })}
                  className="rounded-lg bg-blue-600 px-6 py-2.5 text-sm font-semibold text-white hover:bg-blue-700">{t.button || `Generate Avatar (${CREDIT_COST} credits)`}</button>
                <button onClick={tool.reset}
