...
export default function BackgroundRemoverPage() {
  ...
  useUsageTracker({ toolId: TOOL_ID, toolName: TOOL_NAME, icon: "✂️", creditsUsed, trigger: creditsUsed > 0 });
  ...
  function reset() { setPreview(null); setFile(null); setResultUrl(null); setStatus("idle"); setErrorMsg(""); setMaskPixels(0); }
        ...
        {!preview ? (
          ...
          <div className="space-y-6">
            <div className="grid gap-6 sm:grid-cols-2">
              ...
              <div><p className="mb-2 text-sm font-medium text-zinc-500">Result</p><img src={resultUrl} alt="Result" className="w-full rounded-xl object-contain border checkerboard"/></div>
            </div>
            <div className="flex gap-3"><a href={resultUrl} download target="_blank" rel="noopener noreferrer" className="rounded-lg bg-green-600 px-6 py-2.5 text-sm font-semibold text-white hover:bg-green-700">Download</a><button onClick={reset} className="rounded-lg border border-zinc-300 px-6 py-2.5 text-sm font-medium text-zinc-700 hover:bg-zinc-50 dark:border-zinc-700 dark:text-zinc-300">Try Another</button></div>
          </div>
        ) : status === "uploading" ? (
