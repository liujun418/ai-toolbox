...
export default function BackgroundRemoverClient({ locale = "en" as Locale, dict }: { locale?: Locale; dict?: Record<string, unknown> }) {
  ...
  const tool = useTool({
    ...
    buildPrompt: () => mode === "manual" && userPrompt.trim() ? `Keep only: ${userPrompt.trim()}` : undefined,
    getMask: async () => {
  ...
  // Override handleFileChange to reset canvas state
  ...
  const handleUpload = () => {
    tool.handleUpload({ mode, userPrompt });
  };

  // Reset status when switching from manual to auto
  useEffect(() => {
    if (tool.status === "idle") return;
    // Keep uploading/done/error states intact
  }, [tool.status]);
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-12 sm:px-6">
      <div className="mb-8">
        ...
        <p className="mt-2 text-sm text-zinc-500 dark:text-zinc-400">{t.description || "Remove image backgrounds instantly with AI."} <span className="font-semibold text-blue-600">{t.cost || `${CREDIT_COST} credits`}</span>.</p>
      </div>
      ...
      <div className="rounded-2xl border border-zinc-200 bg-white p-6 dark:border-zinc-800 dark:bg-zinc-900">
        {!tool.preview ? (
        ...
        ) : displayStatus === "done" && tool.resultUrl ? (
          <div className="space-y-6">
            <div className="grid gap-6 sm:grid-cols-2">
              ...
              <div><p className="mb-2 text-sm font-medium text-zinc-500">{tp.result || "Result"}</p><img src={tool.resultUrl} alt="Result" className="w-full rounded-xl object-contain border checkerboard"/></div>
            </div>
            <div className="flex gap-3">
              <a href={tool.resultUrl} download target="_blank" rel="noopener noreferrer" className="rounded-lg bg-green-600 px-6 py-2.5 text-sm font-semibold text-white hover:bg-green-700">{t.download || tp.downloadResult || "Download"}</a>
              <button onClick={tool.reset} className="rounded-lg border border-zinc-300 px-6 py-2.5 text-sm font-medium text-zinc-700 hover:bg-zinc-50 dark:border-zinc-700 dark:text-zinc-300">{t.tryAnother || tp.startOver || "Try Another"}</button>
            </div>
          </div>
          ...
          <div className="py-8 text-center"><p className="mb-4 text-sm text-red-500">{tool.errorMsg}</p><div className="flex justify-center gap-3"><button onClick={handleUpload} className="rounded-lg bg-blue-600 px-6 py-2.5 text-sm font-semibold text-white hover:bg-blue-700">{tp.tryAgain || "Try Again"}</button><button onClick={tool.reset} className="rounded-lg border border-zinc-300 px-6 py-2.5 text-sm font-medium text-zinc-700 dark:border-zinc-700 dark:text-zinc-300">{tp.cancel || "Cancel"}</button></div></div>
        ) : (
          <div className="space-y-6">
            <div className="flex gap-2">
              <button onClick={() => { setMode("auto"); setMaskPixels(0); if (drawRef.current) drawRef.current.getContext("2d")?.clearRect(0,0,drawRef.current.width,drawRef.current.height); }} className={`rounded-lg border px-4 py-2 text-sm font-medium transition-all ${mode==="auto"?"border-blue-600 bg-blue-50 text-blue-700 dark:border-blue-500 dark:bg-blue-900/20 dark:text-blue-300":"border-zinc-200 text-zinc-600"}`}>🤖 Auto Detect</button>
              <button onClick={() => setMode("manual")} className={`rounded-lg border px-4 py-2 text-sm font-medium transition-all ${mode==="manual"?"border-green-600 bg-green-50 text-green-700 dark:border-green-500 dark:bg-green-900/20 dark:text-green-300":"border-zinc-200 text-zinc-600"}`}>✋ Manual Keep</button>
            </div>
            ...
            {mode === "manual" && (
              <div>
                ...
                <div className="relative inline-block max-w-full" style={{ lineHeight: 0 }}>
                  ...
                  <canvas ref={drawRef} onMouseDown={startDraw} onMouseMove={doDraw} onMouseUp={(e) => { stopDraw(); updateMaskCount(); }} onMouseLeave={stopDraw} onTouchStart={startDraw} onTouchMove={doDraw} onTouchEnd={(e) => { stopDraw(); updateMaskCount(); }} className="absolute inset-0 cursor-crosshair rounded-xl" style={{ touchAction: "none", width: "100%", height: "100%" }}/>
                </div>
            ...
            {mode === "auto" && tool.preview && <div><p className="mb-2 text-sm font-medium text-zinc-500">{tp.original || "Original"}</p><img src={tool.preview} alt="Original" className="max-h-[380px] rounded-xl border"/></div>}

            <div className="flex gap-3">
              <button onClick={handleUpload} className="rounded-lg bg-blue-600 px-6 py-2.5 text-sm font-semibold text-white hover:bg-blue-700">{t.button || `Remove Background (${CREDIT_COST} credits)`}</button>
