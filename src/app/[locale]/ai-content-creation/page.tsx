...
const TOPIC_ID = "ai-content-creation";

export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const topic = aiTopics.find((t) => t.id === TOPIC_ID);
  const title = `${topic?.title || "AI Content Creation"} — AI ToolBox Online`;
  const desc = topic?.description || "Free AI content creation tools online.";
