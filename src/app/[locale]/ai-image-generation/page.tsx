...
const TOPIC_ID = "ai-image-generation";

export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const topic = aiTopics.find((t) => t.id === TOPIC_ID);
  const title = `${topic?.title || "AI Image Generation"} — AI ToolBox Online`;
  const desc = topic?.description || "Free AI image generation tools online.";
