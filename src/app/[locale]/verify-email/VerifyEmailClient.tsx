...
export default function VerifyEmailClient({ dict }: { dict?: Record<string, unknown> }) {
  ...
  const t = (dict as any)?.verifyEmail || {};
    ...
    return (
      <div className="mx-auto max-w-md px-4 py-16 sm:px-6 text-center">
        <h1 className="text-2xl font-bold tracking-tight text-zinc-900 dark:text-white">
          {t.failed || "Verification Failed"}
        </h1>
        <p className="mt-4 text-sm text-red-600">{error}</p>
        <p className="mt-4 text-sm text-zinc-500 dark:text-zinc-400">
          {t.invalidToken || "The verification link may have expired."}{" "}
