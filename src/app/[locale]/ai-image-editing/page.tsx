...
const TOPIC_ID = "ai-image-editing";

export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const topic = aiTopics.find((t) => t.id === TOPIC_ID);
  const title = `${topic?.title || "AI Image Editing"} — AI ToolBox Online`;
  const desc = topic?.description || "Free AI image editing tools online.";
